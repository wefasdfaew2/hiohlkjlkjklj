/**
 * @ngdoc controller
 * @name CMS.controller:pagesCtrl
 * @description
 * Static pages controller
 */
angular.module('CMS').controller('pagesCtrl', ['$location', '$rootScope', '$scope', '$sce', '$routeParams', '$window', '$q', '$timeout', '$filter', '$interval', 'smoothScroll',  'content', 'Utils', 'WPConfig', 'Config', 'Translator', 'Storage', 'analytics', 'Moment', 'liveChat', function ($location, $rootScope, $scope, $sce, $routeParams, $window, $q, $timeout, $filter, $interval, smoothScroll, content, Utils, WPConfig, Config, Translator, Storage, analytics, Moment, liveChat) {
    'use strict';
    ...
    $scope.openPage = function openPage(page) {
        ...
        $scope.downloadPDFLink = WPConfig.wpUrl + '?json=pdf&pdf=' +  WPConfig.wpBaseHost + '/' + WPConfig.help.pageSlugs[Config.env.lang] + '/' +$scope.selectedPage.slug;
    };
    ...
    $scope.loadHelpPages = function loadHelpPages(forPopup) {
        ...
        if (Utils.isObjectEmpty($rootScope.helpPages)) {
            ...
            content.getPage(WPConfig.help[forPopup ? 'popupPageSlugs' : 'pageSlugs'][$rootScope.env.lang], true).then(function (data) {
                if (!data.data.page) {
                    return;
                }
    ...
    $scope.selectedCurrency = Config.main.registration.defaultCurrency;
...
//------------------------------------------------------                ------------------------------------------------
    ...
    $scope.openHelpPage = function openHelpPage(slug, from) {
        if (Config.main.openHelpAsPopup && from !== 'footer') {
    ...
    $scope.gePageBanners = function gePageBanners(product, containerId) {
        ...
        containerId = containerId || type + WPConfig.bannerSlugs[product][$rootScope.env.lang];
    ...
    $scope.getCasinoBanners = function getCasinoBanners(containerId) {
        containerId = containerId || WPConfig.bannerSlugs.casino[$rootScope.env.lang];
        content.getWidget(containerId).then(function (response) {
            if (response.data && response.data.widgets && response.data.widgets[0]) {
    ...
    $scope.loadPages = function loadPages(product) {
        ...
        $scope.newsPageSlug = WPConfig[product].newsPageSlug ? WPConfig[product].newsPageSlug[$rootScope.env.lang] : null; //slug of page where news will be shown
        if (Utils.isObjectEmpty($scope[product + 'Pages'])) {
            content.getPage((Config.main.enableNewPoker && product === 'poker' ? 'new-' : '') + $scope.pokerType + WPConfig[product].mainPageSlugs[$rootScope.env.lang], true).then(function (data) {
     ...
     */
    $scope.loadTwitterFeed = function loadTwitterFeed(count) {
        count = count || Config.main.twitterFeed.count;
        if ($location.path() !== '/') {
            $timeout(function () {
                $scope.loadTwitterFeed(count);
            }, Config.main.twitterFeed.refreshInterval);
            return;
        }
        content.getTwitterFeed(Config.main.twitterFeed.user[$rootScope.env.lang], count).then(function (response) {
            $scope.tweets = [];
            var toMatch = new RegExp("\\#" + Config.main.twitterFeed.hashTag + "(\\W|$)+", 'ig');
            angular.forEach(response.data, function (tweet) {
                if (tweet.text && tweet.text.match(toMatch)) {
                    var v = tweet.created_at.split(' ');
                    var parseDate = new Date(Date.parse(v[1] + " " + v[2] + ", " + v[5] + " " + v[3] + " UTC"));
                    tweet.created_at = Moment.get(parseDate).unix();
    ...
    $scope.getPokerLeaderboard = function getPokerLeaderboard() {
        content.getPage(WPConfig.poker.leaderboardWidget[Config.env.lang], true).then(function (response) {
    ...
    $scope.getCustomPage = function getCustomPage(srcSection, srcKey, destKey) {
        ...
        content.getPage(WPConfig[srcSection][srcKey][Config.env.lang], true).then(function (response) {
    ...
    var ws;
    for (ws = 0; ws < 12; ws++) {
        $scope.winnerSlides.push({
            won: Math.floor(Math.random() * 100000),
            name: 'Jacob ' + ws,
            image: 'http://fakeimg.pl/250x250/'
        });
    }
