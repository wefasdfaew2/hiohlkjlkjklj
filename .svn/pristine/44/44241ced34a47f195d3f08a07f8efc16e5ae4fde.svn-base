/**
 * @ngdoc controller
 * @name vbet5.controller:explorerCtrl
 * @description
 * Explorer controller
 */
angular.module('vbet5.betting').controller('explorerCtrl', ['$rootScope', '$scope', 'Config', 'Zergling', 'Utils', '$filter', '$location', '$timeout', '$q', 'analytics', 'Storage', 'Translator', 'GameInfo', function ($rootScope, $scope, Config, Zergling, Utils, $filter, $location, $timeout, $q, analytics, Storage, Translator, GameInfo) {
    'use strict';
    ...
    $scope.$on('widescreen.on', function () {
        ...
        isWideScreen = true;
        console.log('explorer - widecreen on', $scope.regionsListMaxVisibleItems);
    });
    ...
    $scope.$on('widescreen.off', function () {
        ...
        createSportsListMoreDropdown();
        isWideScreen = false;
        console.log('explorer - widecreen off', $scope.regionsListMaxVisibleItems);
    });
     ...
     * @ngdoc object
     * @name gameType
     * @propertyOf vbet5.controller:explorerCtrl
    ...
    function updateGames(data, initial) {
        ...
        if (firstSport && firstSport.id !== $scope.selectedSportId && $scope.selectedSportId !== VIRTUAL_SPORT_FAVORITE.id) {
        ...
        var i;
        for (i = 0; i < $rootScope.selectedCompetitions.length; i++) {
        ...
        $rootScope.selectedCompetitions.sort(function (a, b) {
...
//        console.log('selectedCompetitions:', $rootScope.selectedCompetitions, $rootScope.selectedCompetitionsInColumns);
    }
    ...
    $scope.loadSportsList = function loadSportsList() {
        ...
        var request = {'source': 'betting', 'what': {'sport': [], 'game': '@count'}, 'where': {'sport': { 'id': {'@nin': GameInfo.VIRTUAL_SPORT_IDS}}, 'game': (Config.main.enableVisibleInPrematchGames && !Config.env.live ? {'@or': ([{ 'type': Config.env.live ? 1 : 0 }, {'visible_in_prematch': 1, 'type': 1}])} : {'type': Config.env.live ? 1 : 0})}};
        ...
        Zergling.subscribe(
            ...
            .then(function (result) {
                ...
                if (result.data) {
                    ...
                    var sportToSelect, defaultSportListIndex = Config.main.showFavoriteGamesInSportList ? 1 : 0; //when favorites is enabled, it's the first item and actual sports begin from index 1
                    if (!isNaN(searchSportParam)) {
                        ...
                        sportToSelect = Config.main.selectFavoritesByDefault && $rootScope.myGames && $rootScope.myGames.length ? VIRTUAL_SPORT_FAVORITE : $scope.sports_list[defaultSportListIndex];
                    }
        ...
        function doSubscribe() {
            ...
            var requestMyGame = {
                ...
                'what': {
                    ...
                    'competition': [],
                    'game': [
                        ['id', 'start_ts', 'team1_name', 'team2_name', 'team1_external_id', 'team2_external_id', 'type', 'info', 'events_count', 'markets_count', 'extra', 'is_blocked', 'game_number', 'exclude_ids', 'is_stat_available']
                    ],
    ...
    function addPopularGamesSection(sport) {
        ...
        var request = {
            ...
            'what': { game: '@count'},
            'where': {
                'game': (Config.main.enableVisibleInPrematchGames && !Config.env.live ? {'@or': ([{ 'type': Config.env.live ? 1 : 0 }, {'visible_in_prematch': 1, 'type': 1}])} : {'type': Config.env.live ? 1 : 0}),
                sport: {id: sport.id},
                competition: {'favorite': {'@contains': parseInt(Config.main.site_id, 10)}}
            }
        };
        Zergling.get(request).then(function (response) {
            console.log('checking for popular games , response', response);
    ...
    $scope.selectSport = function selectSport(sport) {
        ...
        if (!sport) {
            console.warn('cannot select sport');
            return;
        }
        ...
        if (sport.id === VIRTUAL_SPORT_VIRTUALS.id) {
            $rootScope.modernVirtualSportsSelected = true;
            $rootScope.selectedSportAlias = sport.alias;
            return;
        }
        $rootScope.modernVirtualSportsSelected = false;
        ...
        function subscribeToRegionList() {
            ...
            var request = {
                ...
                'where': {'sport': {'id': $scope.selectedSportId}, 'game': (Config.main.enableVisibleInPrematchGames && !Config.env.live ? {'@or': ([{ 'type': Config.env.live ? 1 : 0 }, {'visible_in_prematch': 1, 'type': 1}])} : {'type': Config.env.live ? 1 : 0})}
            };
            if (Config.env.gameTimeFilter) {
                request.where.game.start_ts = Config.env.gameTimeFilter;
            }
            Zergling.subscribe(
                ...
                updateRegionsList
                    ...
                    } else if ($rootScope.selectedRegionAlias && regionAliasMap && regionAliasMap[$rootScope.selectedRegionAlias] !== undefined) {
    ...
    $scope.selectRegion = function selectRegion(region) {
        ...
        function subscribeToRegionGames() {
            ...
            } else if ($scope.selectedRegionId === VIRTUAL_REGION_WITHVIDEO.id) {
                request.where.game['@or'] = GameInfo.getVideoFilter();
            } else if ($scope.selectedRegionId === VIRTUAL_REGION_POPULAR.id) {
                request.where.competition = {'favorite': {'@contains': parseInt(Config.main.site_id, 10)}};
            }
     ...
     * @methodOf vbet5.controller:explorerCtrl
     * @description exposes {@link vbet5.service:Utils#setJustForMoment Utils.setJustForMoment} method to explorerCtrl's $scope
     *
