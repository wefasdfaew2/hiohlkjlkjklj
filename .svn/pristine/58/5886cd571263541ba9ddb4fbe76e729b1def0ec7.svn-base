            ...
            return {
                ...
                link: function ($scope, element, attrs) {
                    ...
                    function groupNewsInGroups(count) {
                        ...
                        if (recentNews) {
                            $scope.recentNewsList = Utils.groupNewsToGroups(recentNews, groupCount);
                        }
                    }
                    ...
                    function loadNews(count) {
                        ...
                        var type = $routeParams.type ? $routeParams.type + "-" : "",
                            categorySlug = WPConfig[type + $scope.slug][categorySlugKey][$rootScope.env.lang];
                        $scope.newsAreLoading = true;
                        content.getPostsByCategorySlug(categorySlug, count, false, WPConfig.wpPromoUrl).then(function (response) {
                            $scope.newsAreLoading = false;
                            if (response.data && response.data.posts) {
                                recentNews = response.data.posts;
                                var i;
                                for (i = 0; i < recentNews.length; i += 1) {
                    ...
                    $scope.showNews = function showNews(news, groupId) {
                        ...
                        analytics.gaSend('send', 'event', 'news', 'show poker news', {
                            'page': $location.path(),
                            'eventLabel': news.categories[0].title
                        });
