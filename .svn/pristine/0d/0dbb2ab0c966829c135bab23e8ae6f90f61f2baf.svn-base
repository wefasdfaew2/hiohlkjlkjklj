...
VBET5.factory('WS', ['$q', '$rootScope', '$timeout', '$window', 'Config', 'Utils', function ($q, $rootScope, $timeout, $window, Config, Utils) {
    ...
    var wasConnected = false, retryCount = 0;
...
//            console.log('response time:', (new Date() - callbacks[data.rid].time) / 1000, 'sec');
        ...
        } else if (data.data && data.rid === 0) { //subscription update
            if (Config.swarm.debugging) {
    ...
    function get_connection() {
        ...
        if (!connection) {
            ...
            try {
                var websocketUrl = Utils.getWeightedRandom(Config.swarm.websocket).url;
                console.log('websocketUrl selected:', websocketUrl);
            ...
            socket.onclose = function (event) {
                ...
                connected = false;
                console.log('socket closed', event, callbacks, wasConnected, retryCount);
                ...
                if (wasConnected && retryCount < Config.swarm.maxWebsocketRetries) {
                    WS.isAvailable = true;
                    connection = null;
                    retryCount++;
                    console.log('retry count', retryCount, 'retrying in ', Config.swarm.webSocketRetryInterval * retryCount, 'msec');
