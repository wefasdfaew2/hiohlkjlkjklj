angular.module('liveChat', ['ng']).service('liveChat', ['$window', '$rootScope', '$timeout', 'Config', 'Script', function ($window, $rootScope, $timeout, Config, Script) {
    'use strict';
    var sfChatConfig;
    var deskChatData = {};
    ...
    return {
        init: function init(config) {
            if (config.popup) {
                ...
                comm100_lc.src = 'https://chatserver.comm100.com/livechat.ashx?siteId=' + $window.Comm100API.site_id;
            ...
            if (config.liveZillaTrackingServer) {
                ...
                var lzSrc = config.liveZillaTrackingServer + '/server.php?a=' + config.liveZillaTrackingId + '&rqst=track&output=jcrpt&nse='+Math.random();
        ...
        initLiveAgent: function(){
            (function(d, src, c) { var t=d.scripts[d.scripts.length - 1],s=d.createElement('script');s.id='la_x2s6df8d';s.async=true;s.src=src;s.onload=s.onreadystatechange=function(){var rs=this.readyState;if(rs&&(rs!='complete')&&(rs!='loaded')){return;}c(this);};t.parentElement.insertBefore(s,t.nextSibling);})(document,
                ...
                function(e){
                    ...
                    vb.onload = function () {
                        $rootScope.liveAgentButton = LiveAgent.createVirtualButton(Config.main.liveChat.liveAgentID, vb);
                    };
                });
        }
    };
