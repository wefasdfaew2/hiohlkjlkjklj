...
VBET5.directive('flashplayer', ['$window', '$timeout', '$rootScope', 'Translator', function ($window, $timeout, $rootScope, Translator) {
    ...
    return {
        ...
        link: function (scope, element, attrs) {
            var params = {};
            params.movie = "swf/LiveVideo.swf";
            params.quality = "high";
...
//                console.log('\n\n\n------------', scopeId, scope.$id);
            ...
            var embedSwf = function () {
                if ($window.document.getElementById(attrs.id)) { //wait until element becomes available
                    swfobject.embedSWF("swf/LiveVideo.swf", attrs.id, "100%", "100%", "11.1", "swf/playerProductInstall.swf", {}, params, attributes, function (obj) {
                        if (obj.success === false) {
                            angular.element($window.document.getElementById(attrs.id)).html('<div trans>Please update your flash plugin</div>');
                            return;
                        }
            ...
            //if ($rootScope.conf.sportsLayout === "classic") {
