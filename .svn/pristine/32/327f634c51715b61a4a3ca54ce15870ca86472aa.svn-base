 ...
 *  cashier controller
 */
VBET5.controller('cashierCtrl', ['$scope', '$rootScope', '$location', 'Config', 'Translator', 'Zergling', function ($scope, $rootScope, $location, Config, Translator, Zergling) {
    'use strict';
    ...
    $scope.transfer = function transfer() {
        ...
        var processResponse = function (response) {
            ...
            if (code === 0) {
                ...
                $scope.message = Translator.get('Your transfer is successfully completed');
            } else {
        ...
        Zergling.get(request, 'transfer')
            .then(processResponse)['catch'](function (reason) {
                ...
                $scope.messageType = 'error';
                $scope.message = Translator.get("Internal Error") + ' (' + reason.code + ')';
                $scope.transferInProgress = false;
            });
    };
    ...
    $scope.changeCashierFormModelFromProduct = function changeCashierFormModelFromProduct(selectedProduct) {
        ...
        case 'Casino':
            $scope.cashierFormModel.toProduct = Config.main.enableTransferToPoker ? 'Poker' : 'Sport';
            break;
        default:
            $scope.cashierFormModel.toProduct = 'Casino';
            break;
    ...
    $scope.getMaxTransferAmount = function getMaxTransferAmount() {
        ...
        case 'Casino':
        case 'Poker':
            $scope.cashierFormModel.amount = $scope.casinoBalance.balance;
            break;
        default:
