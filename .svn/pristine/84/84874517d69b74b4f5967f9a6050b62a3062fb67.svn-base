 ...
 */
CMS.service('content', ['WPConfig', '$http', '$rootScope', function (WPConfig, $http, $rootScope) {
    'use strict';
    var content = {};
    ...
    var newsBaseHost = WPConfig.wpNewsUrl[lang] ? WPConfig.wpNewsUrl[lang].match(/^(https?\:)\/\/(([^:\/?#]*)(?:\:([0-9]+))?)(\/[^?#]*)(\?[^#]*|)(#.*|)$/)[2] : WPConfig.wpNewsBaseHost;
    ...
    content.getSports = function getSports() {
        ...
        return $http.get(newsUrl + '?base_host=' + newsBaseHost + addHttpsFlag(newsUrl) + '&json=get_category_index&parent=' + (WPConfig.news.langRootCategories[lang] ||  WPConfig.news.langRootCategories.eng));
    };
    ...
    content.getRecentNews = function getRecentNews(count, categoryId, customNewsUrl) {
        ...
        return $http.get(requestUrl + '?base_host=' + (customNewsBaseHost || newsBaseHost) + addHttpsFlag(requestUrl) + '&json=get_recent_posts' + countStr + categoryStr  + excludedFields + tags);
    };
     ...
     * @returns {Object} promise
     */
    content.getPostsByCategorySlug = function getPostsByCategorySlug(categorySlug, count, includeTagInRequest, customNewsUrl) {
        var customNewsBaseHost;
        if (customNewsUrl) {
            customNewsBaseHost = customNewsUrl.match(/^(https?\:)\/\/(([^:\/?#]*)(?:\:([0-9]+))?)(\/[^?#]*)(\?[^#]*|)(#.*|)$/)[2];
        }
        ...
        var countStr = count === undefined ? '' : '&count=' + parseInt(count, 10);
        var requestUrl = customNewsUrl || newsUrl;
        return $http.get(requestUrl + '?base_host=' + (customNewsBaseHost || newsBaseHost) + addHttpsFlag(requestUrl) + '&json=get_category_posts&category_slug=' + categorySlug + countStr  + excludedFields + (includeTagInRequest ? tags : ''));
    };
    ...
    content.getNewsById = function getNewsById(id, secret) {
        return $http.get(newsUrl + '?base_host=' + newsBaseHost + addHttpsFlag(newsUrl) + '&json=get_post&id=' + id + (secret ? '&secret=' + secret : '')  + excludedFields);
    };
    ...
    content.getWidget = function getWidget(sidebarId) {
        ...
        var url = WPConfig.wpUrl + '?base_host=' + WPConfig.wpBaseHost +  addHttpsFlag(WPConfig.wpUrl) + '&json=widgets/get_sidebar&sidebar_id=' + sidebarId;
    ...
    content.getPage = function getPage(slug, withChildren, cache) {
        ...
        var url = WPConfig.wpUrl + '?base_host=' + WPConfig.wpBaseHost +  addHttpsFlag(WPConfig.wpUrl) + '&json=get_page&slug=' + slug + childrenParam + excludedFields;
     ...
     * @returns {Object} promise
     */
    content.getTwitterFeed = function getTwitterFeed(username, count) {
        var countParam = count ? '&count=' + count : '';
        var noCacheParam = '&r=' + new Date().getTime() + Math.random() * 10;
        return $http.get(WPConfig.wpUrl + '?json=get-twitter-feed' +  addHttpsFlag(WPConfig.wpUrl) + '&username=' + username + countParam + noCacheParam);
    };
