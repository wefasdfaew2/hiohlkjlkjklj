 ...
 */
angular.module('vbet5.betting').controller('gameCtrl', ['$rootScope', '$scope', '$location', '$filter', '$timeout', '$window', 'Config', 'Zergling', 'Utils', 'X2js', 'GameInfo', 'analytics',  function ($rootScope, $scope, $location, $filter, $timeout, $window, Config, Zergling, Utils, X2js, GameInfo, analytics) {
    'use strict';
    ...
    var lastAnimationStart;
    var lastAnimationType;
    var lastAnimationSide;
    /**
     * @ngdoc method
     * @name generateAnimationEvent
     * @methodOf vbet5.controller:gameCtrl
     * @param {object} theGame object contains animation events
     * @description generates current animation event on the field
     */
    var generateAnimationEvent = function (theGame) {
        var theEvent = theGame.last_event;
        if (theEvent && theEvent.type) {
            if (lastAnimationType === theEvent.type && lastAnimationSide === theEvent.side) {
                return;
            }
            if (theGame.sport.alias === "Soccer") {
                lastAnimationType = theEvent.type;
                lastAnimationSide = theEvent.side;
                lastAnimationStart = new Date().getTime();
                $timeout(function stopAnimation() {
                    if (lastAnimationStart < new Date().getTime() - 29000) {
                        theEvent.type = "stopAnimation";
                    }
                }, 30000);
            } 
            //  if(theGame.sport.alias === "Soccer"){
            $scope.game.animatedEvent = {};
            
            $scope.game.animatedEvent.type = theEvent.type;
            $scope.game.animatedEvent.team = theEvent.side === "1" ? theGame.team1_name : theGame.team2_name;
            $scope.game.animatedEvent.side = theEvent.side === "1" ? "home" : "away";
            //    }
        }
    ...
    */
    ...
    $scope.$on('login.loggedIn', function () { $scope.getVideoData($scope.game); });
...
//                var markets = Utils.getItemBySubItemProperty($scope.game.market, 'type', ['P1XP2', 'P1P2']);
            ...
            if (markets) {
                $scope.game.headerMarket =  Utils.objectToArray(markets)[0];
            ...
            if ($scope.game.open) {
                ...
                var improveNameForThisGame = function improveNameForThisGame(name) {
                    return $filter('improveName')(name, $scope.game);
                };
            ...
            if ($scope.game.type === 1 && $scope.game.open && $scope.game.sport.alias === "Soccer") {
                GameInfo.updateSoccerStatistics($scope.game);
                GameInfo.generateTimeLineEvents($scope.game, $scope);
                if ($scope.game.last_event) {
                    generateAnimationEvent($scope.game);
                }
            } else if ($scope.game.type === 1 && $scope.game.last_event && $scope.game.open && $scope.game.sport.alias === "Tennis") {
                GameInfo.addGameScorePerTeam($scope.game.last_event,$scope.game);
                GameInfo.setTennisCourtSide($scope.game.last_event,$scope.game);
                // generateAnimationEvent($scope.game);
            }
            ...
            if ($scope.game.last_event) {
                $scope.game.last_event.team_name = $scope.game.last_event.side === "1" ? $scope.game.team1_name : $scope.game.team2_name;
                if ($scope.game.last_event.type) {
                    $scope.game.last_event.type = $scope.game.last_event.type.replace(/ /g, '');
                }
            }
            ...
            if (hasVideo && (Config.env.authorized || !$scope.game.last_event) && $scope.game.activeFieldType === undefined && Config.main.videoEnabled) {
    ...
    $scope.$watch('game', processGameData, true);
    /**
     * @ngdoc method
     * @name bet
     * @methodOf vbet5.controller:gameCtrl
    ...
    $scope.openStatistics = function openStatistics(game) {
        $window.open(Config.main.statsHostname + '/' + game.team1_external_id + '/' + game.team2_external_id, game.id, "width=940,height=600,resizable=yes");
    };
