...
/**
 * @ngdoc controller
 * @name CMS.controller:sportNewsCtrl
 * @description
 * Sport news controller
 */
CMS.controller('sportNewsCtrl', ['$rootScope', '$scope', '$sce', '$location', '$window', '$q',  '$anchorScroll', '$timeout', 'WPConfig', 'content', 'Config', 'Utils', 'DomHelper', 'Zergling', 'analytics', 'Translator', function ($rootScope, $scope, $sce, $location, $window, $q, $anchorScroll, $timeout, WPConfig, content, Config, Utils, DomHelper, Zergling, analytics, Translator) {
    'use strict';
    ...
    var configureForWideScreen = function () {
        ...
        if ($scope.recentNews) {
            $scope.recentNewsInGroups = Utils.groupNewsToGroups($scope.recentNews, $scope.newsPerGroup);
        }
     ...
     * @methodOf CMS.controller:sportNewsCtrl
    ...
    function ckeckIfLinkedGameExists(news) {
        if (news.custom_fields.type.length && news.custom_fields.sport.length && news.custom_fields.competition.length && news.custom_fields.region.length && news.custom_fields.game.length) {
            ...
            if (gameId) {
                ...
                request.where.game =  {'id': gameId, 'type': type};
            } else if (competition) {
            ...
            if (response.data && response.data.categories) {
                if (WPConfig.hiddenNewsCategoryIds.length && response.data.categories.length) {
                    angular.forEach(WPConfig.hiddenNewsCategoryIds, function (id) {
                        Utils.removeElementFromArray(response.data.categories, Utils.getArrayObjectElementHavingFieldValue(response.data.categories, 'id', id));
                    });
                }
    ...
    $scope.loadRecentNews = function loadRecentNews(count, sportId, startLooping) {
        ...
        content.getRecentNews(count, sportId).then(function (response) {
            ...
            if (response.data && response.data.posts) {
                ...
                console.log('recent news', $scope.recentNews, 'groups:', $scope.recentNewsInGroups);
                if (response.data.count_total <= $scope.numberOfRecentNews) {
                    $scope.noMoreNews = true;
                } else {
                    $scope.noMoreNews = false;
                }
                if (!Config.main.oldHomepage) {
                    if (timer) {
    ...
    $scope.loopThroughNews = function loopThroughNews(scrolled) {
        ...
        timer = $timeout(function () {loopThroughNews(); }, 3000);
    ...
    function getPromotedGames(gamesOfCompetition) {
        ...
        request.where[promotedObject] = {'promoted': {'@contains': parseInt(Config.main.site_id, 10)}};
    ...
    function rotateBanners(banners, period) {
        ...
        $timeout(function () { rotateBanners(banners, period); }, period);
    }
    ...
    $scope.getHomepageBanners = function getHomepageBanners(containerId) {
        containerId = containerId || WPConfig.bannerSlugs.homepageBanners[$rootScope.env.lang];
    ...
    function rotateFeaturedGames() {
        $timeout(rotateFeaturedGames, Config.main.featuredGamesRotationPeriod);
    ...
    $scope.getSelectedHomepageGames = function getSelectedHomepageGames() {
        content.getPage(WPConfig.bannerSlugs.homepageRotatingBanners[$rootScope.env.lang], true).then(function (data) {
...
//                if (new Date() - Date.parse(page.children[i].modified)  < Config.main.homepageSelectedGameLifetime) {
                ...
                $scope.selectedGameIndex = Math.floor(Math.random() * $scope.selectedHomepageGames.length);
            }
            rotateFeaturedGames();
        });
    };
