    ...
    partner.init = function init() {
        ...
        if ($location.search().callbackName) {
            ...
            if (Config.partner.notifyOnResize) {
                DomHelper.onContentheightChange(function (height, width) {
                    partner.call('windowSize', {height: height, width: width});
        ...
        if (authToken && userId && authToken.length > 1 && userId.length > 0 && authToken.toLowerCase() !== 'anonymous') {
            AuthData.integrationMode = true;
            Config.env.integrationMode = true;
            AuthData.set({auth_token: authToken, user_id: Config.partner.allowStringUserId ? userId : parseInt(userId, 10)});
        }
