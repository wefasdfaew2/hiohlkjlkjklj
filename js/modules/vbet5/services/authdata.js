 ...
 */
VBET5.service('AuthData', ['$window', 'Config', 'Storage', 'ipCookie', function ($window, Config, Storage, ipCookie) {
    'use strict';
    var AuthData = {};
    ...
    AuthData.set = function set(data) {
        if (AuthData.integrationMode) {
            ...
            Storage.set('auth_data', data, Config.main.authSessionLifetime);
            if (Config.main.useAuthCookies) {
                var cookieOptions = {
                    domain: $window.location.hostname.split(/\./).slice(-2).join("."),
                    path: "/",
                    expires: Config.main.authSessionLifetime,
                    expirationUnit: 'seconds'
                };
                ipCookie("auth_data", data, cookieOptions);
            }
        }
    ...
    AuthData.get = function get() {
        return AuthData.integrationMode ? AuthData.partnerAuthData : (Config.main.useAuthCookies ?  ipCookie("auth_data") : Storage.get('auth_data'));
    };
