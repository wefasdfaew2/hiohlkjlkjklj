...
//        }
    ...
    function getSession() {
        ...
        if (!session) {
            ...
            var sessionRequestCmd = { 'command': "request_session", 'params': { 'language': getLanguage(Config.env.lang), 'site_id': Config.main.site_id} };
...
//                    console.log('websocket available');
     ...
     */
    Zergling.login = function login(user) {
        console.log('zergling logging in', user);
        var data;
            ...
            .then(function (response) {
                if (response.data.code === Zergling.codes.OK) {
                    ...
                    if (user && response.data.data.auth_token) {
                        var authData = {auth_token: response.data.data.auth_token, user_id: response.data.data.user_id};
