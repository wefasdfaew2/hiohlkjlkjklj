 ...
 *
 */
VBET5.filter('oddConvert', [ 'Config', function (Config) {
    'use strict';
    ...
    for (i = 0; i < ladder.length; i++) {
        decimals.push(parseFloat(parseFloat(eval(ladder[i])).toFixed(4)));
    }
    ...
    function convert(value, format) {
        ...
        switch (format) {
        case 'decimal':
            return (value !== undefined && value !== '') ? (iValue !== fValue && value.toString().split('.')[1].length > 2) ? (Math.round(value * Math.pow(10, Config.main.roundDecimalCoefficients)) / Math.pow(10, Config.main.roundDecimalCoefficients)) : fValue.toFixed(2) : value;
        case 'fractional':
    ...
    return function (value, format, type) {
        ...
        var cacheKey = Config.env.oddFormat.concat(value);
        if (cache[cacheKey] === undefined) {
            format = Config.env.oddFormat;
