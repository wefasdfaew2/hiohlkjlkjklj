 ...
 */
angular.module('vbet5.betting').controller('classicExplorerCtrl', ['$rootScope', '$scope', '$controller', 'Config', 'Zergling', 'Utils', '$filter', '$location', '$timeout', '$q', 'analytics', 'Storage', 'Translator', 'GameInfo', '$window', 'partner', 'DomHelper', 'Moment', function ($rootScope, $scope, $controller, Config, Zergling, Utils, $filter, $location, $timeout, $q, analytics, Storage, Translator, GameInfo, $window, partner, DomHelper, Moment) {
    'use strict';

    angular.extend(this, $controller('classicGameCtrl', {
        $rootScope: $rootScope,
        $scope: $scope,
        $timeout: $timeout,
        $filter: $filter,
    ...
    var TODAY_BETS = {
        id: -11,
        order: Config.main.showTodayBets,
        game: 0,
        innumerable: true,
        alias: 'todaybets',
        isTodayBets: true,
    ...
    /**
     * Assign  route values to scope vars
     */
    ...
    function checkFavoriteCompetitionsCount() {
        function doSubscribe() {
            ...
            var request = {
                ...
                'where': {
                    'competition': {'favorite': {'@contains': +siteId}}
                }
            };
            Zergling.subscribe(
                request,
    ...
    function updateLeftMenuSports(data, firstTime) {
        ...
        if (!$scope.customSportAliasFilter) {
            ...
            if (Config.main.showPrematchLimit > 0) {
        ...
        if (Config.main.expandFavoriteCompetitions) {
    ...
    $scope.expandLeftMenuRegion = function expandLeftMenuRegion(region, keepItOpen) {
        ...
        if (region.expanded) {
            ...
            if (Config.env.live) {
                request.what.game = ['id', 'start_ts', 'team1_name', 'team2_name', 'type', 'info', 'events_count', 'markets_count', 'extra', 'is_blocked', 'game_number', 'stats', 'exclude_ids', 'tv_type', 'video_id', 'video_id2', 'video_id3'];
    ...
    $scope.loadPrematchMultiView = function loadPrematchMultiView(popular) {
        ...
        function doSubscribe() {
                ...
                'what': {
                    ...
                    game: [
                        ['id', 'start_ts', 'team1_name', 'team2_name', 'team1_external_id', 'team2_external_id', 'type',
                            'info', 'events_count', 'markets_count', 'extra', 'is_blocked', 'exclude_ids', 'is_stat_available', 'game_number']
                    ],
            ...
            if (popular && Config.main.loadPopularGamesForSportsBook.enabled) {
                ...
                request.where[Config.main.loadPopularGamesForSportsBook.level][Config.main.loadPopularGamesForSportsBook.type] = {'@contains': parseInt(siteId, 10)};
            ...
            Zergling.subscribe(
                {
                    ...
                    'what': {
                        ...
                        region: ['id'],
                        'competition': ['id', 'name'],
                        'game': ['id', 'start_ts', 'team1_name', 'team2_name', 'type', 'info', 'events_count', 'markets_count', 'extra', 'is_blocked', 'stats', 'tv_type', 'video_id', 'video_id2', 'video_id3', 'game_number']
                    },
