VBET5.controller('euro88PaymentsCtrl', ['$scope', '$rootScope', '$sce', '$filter', 'Zergling', function ($scope, $rootScope, $sce, $filter, Zergling) {
    'use strict';
            ...
            function (data) {
                ...
                if (data && data.result !== undefined && data.result === 0 && data.details && data.details.fields) {
                    ...
                    if ($scope[command].status === 'error' && $scope[command].msg) {
                        $scope.globalPopup = $scope[command].msg;
                    } else {
                        switch (command) {
