 ...
 */
angular.module('vbet5.betting').controller('overviewCtrl', ['$rootScope', '$scope', 'Config', 'Zergling', 'Utils', '$filter', 'Translator', 'GameInfo', 'Storage', function ($rootScope, $scope, Config, Zergling, Utils, $filter, Translator, GameInfo, Storage) {
    'use strict';

    var overviewSubscribtionId, lastData;
    ...
    function updateOverviewData(data) {
        ...
        //console.log('updateOverviewData', data);
        angular.forEach(data.sport, function (sport) {
            ...
            sport.competitions.sort(Utils.orderSortingFn);
        });
        $scope.allSports = Utils.objectToArray(data.sport).sort(Utils.orderSortingFn);
    ...
    $scope.initOverview = function initOverview() {
        ...
        var request = {
            ...
            'what': {
                ...
                game: [
                    ['id', 'start_ts', 'team1_name', 'team2_name', 'type', 'info', 'events_count', 'markets_count', 'is_blocked', 'video_id']
                ],
        ...
        Zergling.subscribe(
            ...
            .then(function (result) {
                if (result.subid) {
                    overviewSubscribtionId = result.subid;
                }
                if (result.data) {
    ...
    $scope.$on('$destroy', function () {
        Zergling.unsubscribe(overviewSubscribtionId);
    });
