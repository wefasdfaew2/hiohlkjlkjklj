 ...
 */
VBET5.controller('poolBettingCtrl', ['$scope', '$rootScope', '$interval', '$timeout', '$filter', '$q', '$location', 'Utils', 'Zergling', 'Moment', 'analytics', 'Translator', 'Config', function ($scope, $rootScope, $interval, $timeout, $filter, $q, $location, Utils, Zergling, Moment, analytics, Translator, Config) {
    'use strict';
    ...
    $scope.addToBetslip = function addToBetslip() {
        ...
        Zergling
            ...
            .then(processTotBetResults)['catch'](
                function (reason) {
                    $scope.poolBettingProperties.resultType = 'error';
                    $scope.poolBettingProperties.result = Translator.get("Sorry we can't accept your bets now, please try later") + ' (' + reason.code + ')';
